...
float  iva::iva::get_sample_rate(void)
{
...
void iva::iva_optimized::process(void)
{
    ...
    if (current_batch_finish_prepare_flag)
    {
        ...
        phi_tmp=( (  signal_esitimate_fft.adjoint() *  signal_esitimate_fft ).diagonal().real().cwiseSqrt().transpose() ).transpose();
        for (int i=0;i<fft_length/2+1;i++)
        {
            phi_matrix.row(i) = signal_esitimate_fft.row(i).cwiseQuotient(phi_tmp.transpose());
        }
        for (int i=0;i<fft_length/2+1;i++)
        {
            r_matrix[i] = (signal_esitimate_fft.row(i).adjoint()*phi_matrix.row(i)).transpose();
...
Eigen::Matrix<float, Eigen::Dynamic, SOURCE_NUM> iva::iva_optimized::frame_shift(Eigen::Matrix<float, Eigen::Dynamic, SOURCE_NUM> mat)
{
    ...
    for (int i=fft_length-shift_size;i<fft_length;i++)
    {
        mat.row(i) = zero_matrix.transpose();
    }
    return mat;
}

...
%    phi(:,1) = audio_cut1_fft  / sqrt(audio_cut1_fft'*audio_cut1_fft);
%    phi(:,2) = audio_cut2_fft  / sqrt(audio_cut2_fft'*audio_cut2_fft);
    for k = 1:fft_length
       ...
       audio_estimate(k,:) = audio_estimate(k,:) / (abs(G(1,1,k))*abs(G(2,2,k)));
    end
end
